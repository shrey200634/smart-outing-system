...
const ROLES = [
  ...
  { value: "GUARD", label: "Guard", icon: "🛡️", desc: "Verify QR at gate" },
];
...
export default function Register({ onSwitchToLogin }) {
  ...
  const [step, setStep] = useState(1); // 1 = info, 2 = role select
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      // Backend UserCredential: name, email, password, role
      await register({
        name: form.name.trim(),
      ...
      if (msg.toLowerCase().includes("exist") || msg.toLowerCase().includes("duplicate") || msg.includes("1062")) {
        ...
        <div style={styles.stepWrap}>
          {[1, 2].map((s) => (
            <div key={s} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{
                ...styles.stepDot,
                background: step >= s ? "#F6C90E" : "rgba(255,255,255,0.1)",
                color: step >= s ? "#060D1F" : "#4b5563",
              }}>{s}</div>
        ...
        {step === 1 ? (
          <div style={styles.form}>
            ...
            <div style={styles.fieldWrap}>
              ...
              <div style={{ position: "relative" }}>
                <span style={{ ...styles.inputIcon, color: focused === "confirm" ? "#F6C90E" : "#6b7280" }}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {ROLES.map((r) => (
              ...
              >
                ...
                <div style={{ textAlign: "left" }}>
                  <div style={{ color: form.role === r.value ? "#F6C90E" : "#f9fafb", fontWeight: 700, fontSize: 15, fontFamily: "'Syne', sans-serif" }}>
                    {r.label}
                  </div>
...
const styles = {
  ...
  btn: {
    padding: "15px 24px", background: "linear-gradient(135deg, #F6C90E 0%, #E8A000 100%)",
    border: "none", borderRadius: 12, color: "#060D1F", fontSize: 14, fontWeight: 700,
    fontFamily: "'Syne', sans-serif", cursor: "pointer", width: "100%",
    boxShadow: "0 8px 24px rgba(246,201,14,0.25)", transition: "transform 0.15s",
  },
  backBtn: {
  ...
  spinner: {
    ...
    borderTopColor: "#060D1F", borderRadius: "50%",
    animation: "spin 0.7s linear infinite", display: "inline-block",
  },
  linkBtn: {
    background: "none", border: "none", color: "#F6C90E",
