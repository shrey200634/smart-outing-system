...
export default function Login({ onSwitchToRegister }) {
  const { login, setUserRole, getRoleForUser } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const role = getRoleForUser(form.username.trim());
      if (role) setUserRole(role);
      toast(`Welcome back, ${form.username}! 🎉`, "success");
    } catch (err) {
      const msg = err.message || "Login failed";
      if (msg.includes("User not found")) toast("No account found with this username", "error");
      else if (msg.includes("Wrong Password") || msg.includes("Invalid Access")) toast("Incorrect password. Try again.", "error");
      else if (msg.includes("fetch") || msg.includes("Failed to fetch")) toast("Cannot connect to server. Is the backend running on port 8989?", "error");
...
const styles = {
  ...
  logoTitle: {
    ...
    fontSize: 18, fontWeight: 700, color: "#F6C90E", letterSpacing: "-0.3px",
  },
  logoSub: {
    fontSize: 11, color: "#6b7280", letterSpacing: "0.5px", marginTop: 1,
  },
  heading: {
    fontFamily: "'Syne', sans-serif",
    fontSize: 28, fontWeight: 800, color: "#f9fafb", letterSpacing: "-0.5px", marginBottom: 8,
  },
  subheading: {
    fontSize: 14, color: "#6b7280", marginBottom: 32, lineHeight: 1.5,
  },
  ...
  btn: {
    ...
    justifyContent: "center",
    letterSpacing: "0.3px",
    boxShadow: "0 8px 24px rgba(246,201,14,0.3)",
  ...
  dividerText: {
    fontSize: 13, color: "#4b5563", whiteSpace: "nowrap",
  },
  switchBtn: {
    width: "100%",
    padding: "14px 24px",
    background: "rgba(246,201,14,0.07)",
    border: "1.5px solid rgba(246,201,14,0.2)",
    borderRadius: 12,
    color: "#F6C90E",
    fontSize: 14,
    fontWeight: 600,
