...
export default function StudentPortal() {
  ...
  const [form, setForm] = useState({
    studentId: "", studentName: user?.name || "", parentEmail: "",
    reason: "", destination: "",
    outDate: "", returnDate: "",
  });
  ...
  const loadHistory = async () => {
    ...
    setLoading(true);
    try {
      const data = await outingAPI.getStudentHistory(form.studentId);
      setHistory(data);
    } catch (err) {
  ...
  const handleApply = async (e) => {
    ...
    try {
      ...
      const result = await outingAPI.apply(payload);
      toast("Outing request submitted! AI analysing... 🤖", "success");
      setForm({ studentId: "", studentName: user?.name || "", parentEmail: "", reason: "", destination: "", outDate: "", returnDate: "" });
      setTab("history");
      setTimeout(() => loadHistory(), 500);
    } catch (err) {
      const msg = err.message || "Submission failed";
      if (msg.includes("active") || msg.includes("approved")) toast("You already have an active outing request.", "warn");
      else if (msg.includes("banned") || msg.includes("Denied") || msg.includes("overdue")) toast(msg, "error");
        ...
        <div style={styles.sideTop}>
          ...
          <nav style={styles.nav}>
            {[
              ...
              <button
                key={item.id}
                onClick={() => { setTab(item.id); if (item.id === "history" && form.studentId) loadHistory(); }}
          ...
          <div style={styles.content}>
            ...
            <form onSubmit={handleApply} style={styles.formCard}>
              <div style={styles.formGrid}>
                <Field label="Student ID" name="studentId" value={form.studentId} onChange={handleChange} placeholder="e.g. STU2024001" icon="🆔" />
                <Field label="Full Name" name="studentName" value={form.studentName} onChange={handleChange} placeholder="Your full name" icon="👤" />
                <Field label="Parent Email" name="parentEmail" value={form.parentEmail} onChange={handleChange} type="email" placeholder="parent@email.com" icon="📧" span={2} />
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  <textarea
                    ...
                    placeholder="Describe why you need to go out... (AI will analyse urgency)"
                    rows={3}
          ...
          <div style={styles.content}>
            <div style={styles.pageHeader}>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                ...
                />
                <button onClick={loadHistory} style={styles.searchBtn}>
            ...
            {history.length === 0 && !loading && (
              <div style={styles.emptyState}>
                ...
                <div style={{ color: "#6b7280", fontSize: 13, marginTop: 6 }}>Enter your Student ID above or apply for your first outing.</div>
              </div>
            )}

            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {history.map((o) => (
                <div key={o.id} onClick={() => setSelectedOuting(o)} style={styles.historyCard}>
                  ...
                  {o.qrCodeUrl && o.status === "APPROVED" && (
                    <div style={{ marginTop: 12, padding: 12, background: "rgba(16,185,129,0.06)", borderRadius: 10, border: "1px solid rgba(16,185,129,0.15)" }}>
                      <div style={{ color: "#10b981", fontSize: 12, fontWeight: 600, marginBottom: 8 }}>✓ APPROVED — Show this QR to guard</div>
...
const styles = {
  layout: {
    display: "flex", minHeight: "100vh", background: "#060D1F",
    fontFamily: "'DM Sans', sans-serif",
  },
  sidebar: {
    width: 260, background: "rgba(10,18,40,0.95)", borderRight: "1px solid rgba(255,255,255,0.06)",
  ...
  avatar: {
    ...
    color: "#060D1F", fontWeight: 800, fontSize: 15,
    display: "flex", alignItems: "center", justifyContent: "center",
  ...
  navBtn: {
    ...
    color: "#6b7280", fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif",
    transition: "all 0.2s", textAlign: "left",
  },
  navBtnActive: {
    background: "rgba(246,201,14,0.1)", color: "#F6C90E",
  ...
  pageTitle: {
    fontFamily: "'Syne', sans-serif", fontSize: 26, fontWeight: 800, color: "#f9fafb",
  },
  pageDesc: { color: "#6b7280", fontSize: 14, marginTop: 4 },
  formCard: {
    background: "rgba(10,18,40,0.8)", border: "1px solid rgba(255,255,255,0.08)",
  ...
  aiNote: {
    ...
    padding: "12px 16px", background: "rgba(246,201,14,0.05)",
    border: "1px solid rgba(246,201,14,0.12)", borderRadius: 10, flex: 1,
  },
  submitBtn: {
    padding: "14px 28px", background: "linear-gradient(135deg, #F6C90E, #E8A000)",
    border: "none", borderRadius: 12, color: "#060D1F", fontSize: 14,
    fontWeight: 700, fontFamily: "'Syne', sans-serif", cursor: "pointer",
    whiteSpace: "nowrap", boxShadow: "0 6px 20px rgba(246,201,14,0.3)",
  },
  searchInput: {
    padding: "10px 16px", background: "rgba(255,255,255,0.04)",
    border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 10,
    color: "#f9fafb", fontSize: 14, fontFamily: "'DM Sans', sans-serif",
    outline: "none", width: 220,
  },
  searchBtn: {
    padding: "10px 20px", background: "rgba(246,201,14,0.1)",
    border: "1.5px solid rgba(246,201,14,0.25)", borderRadius: 10,
    color: "#F6C90E", fontSize: 13, fontWeight: 600,
    fontFamily: "'Syne', sans-serif", cursor: "pointer",
  },
  emptyState: {
    textAlign: "center", padding: "80px 20px",
    background: "rgba(10,18,40,0.5)", borderRadius: 20,
